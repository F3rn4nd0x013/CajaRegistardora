...
/*void crearVenta(){
    ...
    for (int f = 0; f < cantPro; f++){
        ...
        if (!buscarPosicion(codigo, &aux) >= 0)
        {
            ...
            fprintf(archProducto, " %.d", (int)prod.cant);
            printf("Quedan en total %.d\n", (int)prod.cant);
        }
        fprintf(archVentas, "%i %s %.f %i %d\n", prod.id, prod.nombre, prod.precio, cantidad, cantPro);
        printf("Se ha agregado la venta al archivo de ventas.\n");
        getch();
    }
    fclose(archProducto);
    fclose(archVentas);
...
void crearVenta(){
    ...
    do{
        ...
        if (!buscarPosicion(codigo, &aux) >= 0)
        {
            ...
            fseek(archProducto, aux.id + 2 + strlen(prod.nombre) + 1 + strlen(lenPrecio) + 1, SEEK_SET);
            fprintf(archProducto, " %.f", prod.cant);
        ...
        printf("¿Desea llevar otro producto? (s/n): ");
        scanf("%c", &llevarOtroProducto);
        if (llevarOtroProducto == 's' || llevarOtroProducto == 'S'){
            continue;
        }else
        {
...
/*void imprimirVentas()
{
    ...
    while (fgets(linea, 100, fp))
    {
        ...
        if (count == 3)
        {
            ...
            // printf("La linea tiene exactamente 3 elementos separados por espacio\n");
            sscanf(buffer, "\n%d %s %s\n", &venta[contador].codigo, venta[contador].fecha, venta[contador].nombreC);
        }
        else if (count == 5)
        {
            // printf("La linea no tiene exactamente 3 elementos separados por espacio\n");
...
void imprimirVentas()
{
    ...
    int cantProductos = 0;
    int i, j;
    float precioTotal, precioProducto;
    ...
    for (i = 1; i < contador + 1; i++)
    {
        ...
        for (j = 0; j < venta[i].numProductos; j++)
        {
            venta[i].lista[j].precio *= venta[i].lista[j].cant;
            printf("\tProducto #%d: %s, Cantidad: %.f, Precio Unitario: %.f\n",
                    venta[i].lista[j].id, venta[i].lista[j].nombre, venta[i].lista[j].cant, venta[i].lista[j].precio);
            precioTotal += venta[i].lista[j].precio;
        }
        printf("\t-----------------------------");
        printf("\n\tPrecio Total: %.f\n", precioTotal);
        printf("\t-----------------------------\n");
